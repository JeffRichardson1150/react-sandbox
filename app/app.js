    ...
    <header>
        <form id="event-search">
          <div class="text-input">
            <input id="event-location" class="form-control" type="text" name="location" value="" placeholder="(e.g. New York or 10018)">
          <div class="text-input">
            <input id="event-keyword" class="form-control" type="text" name="keyword" value="" placeholder="(e.g. Artist or Keyword)">
          <button id="event-submit" type="submit">Submit</button>
    <div id="ytplayer">Search for events and browse video previews!</div>
    <main>
      <div id="content"></div>
    </main>
    <footer>
...
$("#event-search").submit(function(e) {  // $ calls on the DOM element event-search, a form id in index.html
  e.preventDefault();
...
function receiveEvents(events) {
  ...
  events.forEach(function(event){
    ...
    $.get("tpl/item.html", function(data) {  // item.htl formats each event in the result set in a vertical list
      $li.prepend(tplawesome(data, [{   // tplawesome is at top - formats res from inputs (e,t)
        event_url: event.url,
        image_url: event_img,
        title: event.title,
