...
useEffect(() => {
  console.log(`****************** in the useEffect.  gonna set EVDB to window.EVDB. Right now, EVDB = ${EVDB}`)
  setEVDB(window.EVDB)
  console.log(`****************** Just set EVDB in the useEffect.  to window.EVDB. now, EVDB = ${EVDB}`)
...
const fetchResults = () => {
  ...
  console.log(`***************** in fetchResults checking whether EVDB is true. EVDB had been set in the useEffect.  EVDB = ${EVDB}`)

  if(EVDB) {
    console.log(`*********** EVDB was found to be true. gonna do EVDB.API.call. EVDB = ${EVDB}`)
    EVDB.API.call("/events/search", _oArgs, function(oData) {
      console.log(oData)
    })
  } else {
    console.log(`*********** EVDB was found to be false. didn't do EVDB.API.call. EVDB = ${EVDB}`)
    ...
    {/* <script src="http://api.eventful.com/js/api" type="javascript"></script> */}
        {console.log(`******************** in the return. EVDB = ${EVDB}`)}
